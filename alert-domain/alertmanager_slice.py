...
def time_change(start_hour, start_min, end_hour, end_min):
    ...
    # 构造北京时间的今天23:00
    start_bj_time = datetime(current_time.year, current_time.month, current_time.day, start_hour, start_min, 0)
...
def set_silence(job_name, project_name, start_time, end_time):
    data = {
        "matchers": [
            {
                "name": "job",
...
def job():
    ...
    for _, row in df.iterrows():
        print(row['项目名'])
        set_silence(job_name, row['项目名'], *time_change(row['静默开始时间'], start_min, row['静默结束时间'], end_min))
